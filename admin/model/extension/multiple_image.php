<?php
class ModelExtensionmultipleimage extends Model {
		...
		if($photo_info) {
			...
			if(!empty($upload_files)) {
				...
				foreach($upload_files as $image) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "album_photos_image SET album_photos_id = '" . $this->db->escape($album_photos_id) . "', album_id = '" . $this->db->escape($photo_info['album_id']) . "', image = '" . $this->db->escape('upload/'. $image) . "', sort_order = '" . $this->db->escape($j) . "'");
					$j++;
				}
			}
		}
	...
	public function getMultipleImages($album_photos_id) {
		$query = $this->db->query("SELECT * FROM ". DB_PREFIX ."album_photos_image WHERE album_photos_id = '". (int)$album_photos_id ."' ORDER BY sort_order ASC ");
	...
	public function getLastSortOrderMultipleImages($album_photos_id) {
		$query = $this->db->query("SELECT sort_order FROM " . DB_PREFIX . "album_photos_image WHERE album_photos_id = '". (int)$album_photos_id ."' ORDER BY sort_order DESC LIMIT 0,1");
