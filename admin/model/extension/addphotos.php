<?php
class ModelExtensionaddphotos extends Model {
	 ...
	 $upload_files = array();
			...
			if(!empty($upload_files)) {
				foreach($upload_files as $image) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "album_photos SET 
						album_id = '" .$data['album_id'] . "', sort_order = '" . (int)$data['sort_order'] . "',
						image = '" . $this->db->escape('catalog/upload/'. $image) . "'");
	...
	public function getaddphotoss($data = array()){
		$sql = "SELECT *,apd.name as adname,ade.name as album_name  FROM " . DB_PREFIX . "album_photos as ap Left join " .DB_PREFIX. "album_photo_description AS apd on(ap.album_photos_id=apd.album_photos_id) LEFT JOIN ".DB_PREFIX."album_description ade ON (ade.album_id=ap.album_id) WHERE ade.language_id='".$this->config->get('config_language_id')."' AND apd.language_id='".$this->config->get('config_language_id')."' AND ap.album_id>0";
	...
	public function getTotaladdphotoss($data = array()) {
      	$sql ="SELECT COUNT(*) AS total FROM " . DB_PREFIX . "album_photos as ap Left join " .DB_PREFIX. "album_photo_description AS apd on(ap.album_photos_id=apd.album_photos_id) LEFT JOIN ".DB_PREFIX."album_description ade ON (ade.album_id=ap.album_id) WHERE ade.language_id='".$this->config->get('config_language_id')."' AND apd.language_id='".$this->config->get('config_language_id')."' AND ap.album_id>0";
